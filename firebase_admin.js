...
function loadAdminsList() {
  onSnapshot(collection(db, 'users'), (snapshot) => {
    ...
    snapshot.forEach(docSnap => {
       ...
       if(u.role === 'superadmin') { statusHtml = `<span style="color:#C9A84C; font-weight:bold;">مدير عام</span>`; actionHtml = `<span style="color:var(--silver);">صلاحيات كاملة</span>`; } 
       ...
       else { statusHtml = `<span style="color:#3b82f6; font-weight:bold;">مشرف عادي</span>`; actionHtml = `<button onclick="window.toggleAdminRole('${uid}', 'suspended')" style="background:#ef4444; color:white; border:none; padding:5px 10px; border-radius:4px; cursor:pointer; margin-left:5px;">حظر</button><button onclick="window.toggleAdminRole('${uid}', 'superadmin')" style="background:#C9A84C; color:var(--dark); font-weight:bold; border:none; padding:5px 10px; border-radius:4px; cursor:pointer;">ترقية لمدير 👑</button>`; }
...
window.toggleAdminRole = async function(uid, newRole) {
  ...
  try { await updateDoc(doc(db, 'users', uid), { role: newRole }); logActivity(`${actionText} مشرف`, `المعرف: ${uid}`); } 
